...
<div class="w-full mx-auto overflow-y-hidden max-w-7xl" x-data="{ view: 'table' }">
    ...
    <flux:tabs variant="segmented" size="sm" class="mt-4" x-model="view">
        <flux:tab icon="squares-2x2" name="grid">Grid</flux:tab>
        <flux:tab icon="list-bullet" name="table">Table</flux:tab>
    </flux:tabs>

    <div x-cloak x-show="view === 'grid'" class="rounded-xl bg-slate-50 dark:bg-slate-900 p-1 shadow-inner mt-4">
        ...
        <div class="rounded-lg rounded-b-lg bg-white dark:bg-slate-800/50 mt-5 shadow-xs-with-border px-3.5 m-0.5"
            ...
            <div class="pt-2 @if ($vault_records->count() > 9) pb-2 @endif">
                {{-- {{ $vault_records->links() }} --}}
            </div>
        </div>
    ...
    <div x-show="view === 'table'" class="rounded-xl bg-slate-50 dark:bg-slate-900 p-1 shadow-inner mt-4">
        <div class="rounded-lg rounded-b-lg bg-white dark:bg-slate-800/50 shadow-xs-with-border px-3.5 m-0.5">
            <div class="p-2">
                <flux:table>
                    <flux:table.columns>
                        ...
                        <flux:table.column sortable :sorted="$sort_by === 'id'" :direction="$sort_direction"
                            wire:click="sort('id')">
                            ID
                        </flux:table.column>

                        <flux:table.column sortable :sorted="$sort_by === 'vault_id'" :direction="$sort_direction"
                            wire:click="sort('vault_id')">
                            Vault ID
                        </flux:table.column>

                        <flux:table.column sortable :sorted="$sort_by === 'imdb_id'" :direction="$sort_direction"
                            wire:click="sort('imdb_id')">
                            IMDB ID
                        </flux:table.column>

                        <flux:table.column sortable :sorted="$sort_by === 'title'" :direction="$sort_direction"
                            wire:click="sort('title')">
                            Title
                        </flux:table.column>

                        <flux:table.column sortable :sorted="$sort_by === 'rating'" :direction="$sort_direction"
                    ...
                    <flux:table.rows wire:sortable="updateVaultOrder" wire:sortable.options="{ animation: 100 }">
                        @foreach ($vault_records as $vault)
                            <flux:table.row wire:sortable.item="{{ $vault->id }}"
                                wire:key="vault-{{ $vault->id }}" class="bg-white dark:bg-slate-900/50">
                                <flux:table.cell wire:sortable.handle>
                                    <flux:icon.arrows-up-down class="w-4 h-4 stroke-2" />
                                </flux:table.cell>
